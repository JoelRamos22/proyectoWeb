...
export class allPage extends LitElement {
    ...
    guardarProducto(event) {
        const producto = {
            nombre: event.target.closest('.product').querySelector('.product-description').textContent,
            precio: parseFloat(event.target.closest('.product').querySelector('.product-price').textContent.replace('$', '')),
        };
                ...
                <aside class="aside">
                    ...
                    <nav>
                        <ul class="list">
                            ...
                            <li>
                                <button class="boton_categoria_cart-fill ${this.botonActivo === 'carrito' ? 'boton-activo' : ''}" @click=${() => this.eliminarProductos()}>
